                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-700/50 border-b border-slate-600">
                                ...
                                <th class="px-6 py-4 font-semibold uppercase text-xs text-slate-400 tracking-wider">Kategorie</th>
                        ...
                        <tbody class="divide-y divide-slate-700">
                            <?php foreach ($books as $book): ?>
                                <tr class="hover:bg-slate-700/30 transition-colors group">
                                    <td class="px-6 py-4 text-center text-slate-300"><?= $book['id'] ?></td>
                                    <td class="px-6 py-4 text-slate-200"><?= htmlspecialchars($book['title']) ?></td>
                                    <td class="px-6 py-4 text-slate-400"><?= htmlspecialchars($book['author']) ?></td>
                                    <td class="px-6 py-4 text-emerald-400 font-medium">
                                        <?= htmlspecialchars($book['category_name'] ?? 'Nezařazeno') ?>
                                    </td>
                                    <td class="px-6 py-4 text-slate-400"><?= $book['year'] ?></td>
                                    <td class="px-6 py-4 text-right text-slate-300"><?= $book['price'] ?> Kč</td>
